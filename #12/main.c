...
int *get_font(int no, int color)
{
	int *font, i, j;
	int temp;
	font = (int *)malloc(helvB12_width[no] * CHAR_HEIGHT * sizeof(int)*4);
	for (i = 0; i < 2*CHAR_HEIGHT; i++)
	{
		temp = helvB12_bits[no * CHAR_HEIGHT + i/2];
		for (j = 0; j < 2*helvB12_width[no]; j++)
		{
			if (temp & (1 << (16-j/2)))
			{
...
void print_a_word(int x, int y, int charno, int color, int *buffer)
{
	...
	int *font,tempx = x-10-helvB12_width[charno],tempy = y-8,i,j;
	
	
	font = get_font(charno, color);
	for(i=0;i<2*CHAR_HEIGHT;i++)
	{
		for(j=0;j<2*helvB12_width[charno];j++)
		{
			if (i < 0 || i > box.height || j < 0 || j > box.width)
				continue;
			if(font[i * helvB12_width[charno]*2 + j])
			{
...
int main()
{
	int i = 0, j = 0, k = 0, l = 0, file_key, pressed = 0, function = 0, mode = 0, charno = 83;
